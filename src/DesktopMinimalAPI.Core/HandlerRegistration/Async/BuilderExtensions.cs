...
public static class BuilderExtensions
{
    ...
    public static HandlerBuilderBase MapGet<TP, TIn, TOut>(this HandlerBuilderBase builder, string route, Func<TP, Task<TOut>> handler)
        where TP : ParameterSource<TIn> =>
     ApplyRoute<Func<WmRequest, Task<WmResponse>>>(route, builder.MapGet)(Transform<TP, TIn, TOut>(handler));

    public static HandlerBuilderBase MapGet<TP1, TP2, TIn1, TIn2, TOut>(this HandlerBuilderBase builder, string route, Func<TP1, TP2, Task<TOut>> handler)
        where TP1 : ParameterSource<TIn1>
        where TP2 : ParameterSource<TIn2> =>
     ApplyRoute<Func<WmRequest, Task<WmResponse>>>(route, builder.MapGet)(Transform<TP1, TP2, TIn1, TIn2, TOut>(handler));
